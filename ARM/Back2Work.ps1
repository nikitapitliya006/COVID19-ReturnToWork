...
$outputs=New-AzResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile "DeployReturn2WorkAssets.json" -administratorLogin $adminUser -administratorLoginPassword $adminPassword -databaseName $databaseName
...
$outputs=New-AzResourceGroupDeployment -ResourceGroupName $resourceGroupName -TemplateFile "DeployReturn2WorkAzureFunctions.json"
foreach($key in $outputs.Outputs.keys){
...
az functionapp deployment source config-zip -g $resourceGroupName -n $appName --src "Back2WorkAzureFunctions.zip"
